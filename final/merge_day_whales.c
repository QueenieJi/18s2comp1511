...
void merge_day_whales(struct pod *first_pod) {
    struct pod *pre = first_pod;
    struct pod *cur = pre->next;
    struct pod *temp = first_pod;
    while (temp != NULL) {
        pre = temp;
        cur = pre->next;
        while(cur != NULL) {
            if (cur->when->year == temp->when->year && cur->when->month == temp
                ->when->month && temp->when->day == cur->when->day && strcmp(cur->species, temp->species) == 0) {
                temp->how_many = temp->how_many + cur->how_many;
                pre->next = cur->next;
                free(cur->when);
                free(cur->species);
                free(cur);
