...
static void checkGameState(Game game, int expectedPlayer, int expectedTurn,
    ...
    printf("getTopDiscardTurnNumber(game) is %d\n\n",getTopDiscardTurnNumber(game));
...
static void checkTopDiscard(Game game, Card card){
	Card c = getDiscardPileCard(game, 0);
	printf("color%d,value%d,suit%d address%p\n",cardColor(c),cardValue(c),cardSuit(c),c);
	printf("color%d,value%d,suit%d address%p\n",cardColor(card),cardValue(card),cardSuit(card),card);
    assert(c == card);
}
