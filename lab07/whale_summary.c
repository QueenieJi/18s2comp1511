...
void print_tally(struct tally tally) {
	printf("%d %s pods containing %d whales\n", tally.pods, tally.species, tally.whales);
}

void whale_summary(int n_sightings, struct pod sightings[n_sightings]) {
	int line = 0;
	struct tally tallies[256];
	int tally_len = 0;
	int i = 0;//species
	while (line < n_sightings) {
		i = 0;
		while (i < tally_len && strcmp(sightings[line].species, tallies[i].species) != 0) {
			i++;
		}
		if (i == tally_len) {
			tally_len++;
			tallies[i].pods = 1;
			tallies[i].whales = sightings[line].how_many;
			strcpy(tallies[i].species, sightings[line].species);
		} else {
			tallies[i].whales += sightings[line].how_many;
			tallies[i].pods += 1;
		}
