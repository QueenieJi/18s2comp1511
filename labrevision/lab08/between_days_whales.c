...
int date_comparison(struct date *start_date,struct date *finish_date,
...
void between_days_whales(struct pod *first_pod, struct date *start_day, struct date *finish_day) {
	...
	while (curr != NULL) {
		if (date_comparison(start_day,finish_day,curr->when) == 1) {
			printf("%02d/%02d/%02d %d %s",curr->when->day,curr->when->month,
				   curr->when->year,curr->how_many,curr->species);
		}
		curr = curr->next;
	} 
...
int date_comparison(struct date *start_date,struct date *finish_date,
					struct date *cur_date) {
	int re_value = FALSE;
	int num_start = start_date->year * 10000 + start_date->month * 100 +
					start_date->day;
	int num_finish = finish_date->year * 10000 + finish_date->month * 100 +
					finish_date->day;
	int num_curr = curr_date->year * 10000 + curr_date->month * 100 +
					curr_date->day;
	if (num_curr <= num_finish && num_curr >= num_start){
		re_value = TRUE;
	}				
