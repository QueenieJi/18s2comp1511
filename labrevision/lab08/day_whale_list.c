...
void day_whales(struct pod *first_pod, struct date *day) {
	...
	while (curr != NULL){
		if (day_comparison(curr,day)) {
			
			printf("%d %s\n",curr->how_many,curr->species);
		}
		curr = curr->next;
	}
...
int day_comparison(struct pod *first_pod, struct date *day) {
	int return_value = TRUE;
	if (first_pod->when->year != day->year) {
		return_value = FALSE;
	} else if (first_pod->when->month != day->month){
		return_value = FALSE;
	} else if (first_pod->when->day != day->day){
		return_value = FALSE;
	}
	return return_value;
}
