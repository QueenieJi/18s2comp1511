...
// These are the straight lines of numbers, with the first number coming from
// the line before.
// There are 4 cases to deal with each line based on which direction it would go
// when spiraling inwards.

// (For the example below 'r' is for lines going right,
//  'd' for lines going down, 'u' for lines going up,
//  and 'l' for lines going left)
//
// eg.
//
//  rrrrr
...
int main(void) {
    ...
    // longer than the others since it doesn't have another direction that
    // takes its first number.
    // The up line starts shorter than the others since every time it gets to
    // the upwards line the line gets smaller.
    ...
    while (up_size > 0) {
        ...
        // Every go of the spiral, each side shrinks by 4
        up_size = up_size - 4;
    }
    while (down_size > 0) {
    ...
    while (row < num) {
        ...
        while (col < num) {
            ...
            // The number of line changes in the spiral before this line
            int line_num = 0;

            // The number along the line this position is
            int offset = 0;

            // The direction the line's going (or none if a dash should be printed)
            int line_dir = DIR_NONE;


            if (row <= num/2 && row % 2 == 0 && col >= row - 1 && col <= num-row - 1) {
            ...
            } else if (row > num/2 && row % 2 == 0 && col <= row && col >= num - row - 1) {
            ...
            } else if (col <= num/2 && col % 2 == 0 && row >= col + 2 && row < num - col - 1) {
                // Line going upwards
            ...
            } else if (col > num/2 && col % 2 == 0 && row <= col && row >= num - col) {
            ...
            if (line_dir != DIR_NONE) {
                ...
                // The total numbers that have been printed so far in the spiral
                int num_so_far = 0;
                int i = 0;
                ...
                // Include the amount printed in the current line
                num_so_far = num_so_far + offset;

                // Subtract num_so_far from total_stars as
                // the spiral should be counting down as it goes inwards
                int to_print = total_stars - num_so_far;

                // Only take last digit of what to print
                printf("%d", to_print % 10);

            } else {
                // Not part of spiral, just print a dash
                printf("-");
            }
            col = col + 1;
        }
